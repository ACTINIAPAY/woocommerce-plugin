...
/**
 * WC_Actinia_Subscriptions class.
 *
 * @extends WC_Actinia
 */
class WC_Actinia_Subscriptions extends WC_Actinia
{

    /**
     * WC_Actinia_Subscriptions constructor.
     */
    ...
    public function scheduled_subscription_payment($amount_to_charge, $renewal_order)
    {
        ...
        if (is_wp_error($response)) {
            $renewal_order->update_status('failed', sprintf(__('Payment failed (%s)', 'actinia-woocommerce-payment-gateway'), $response->get_error_message()));
        }
    }
    ...
    public function process_subscription_payment($order, $amount = 0)
    {
        ...
        $token = get_user_meta($customerId, 'actinia_token');
        ...
        if ($this->checkToken($token[0])) {
            ...
            $result = $this->do_subscription_payment($actinia_args, $order);
            if ($result['response']['response_status'] == 'failure') {
                ...
                $order->update_status('failed', sprintf(__('Payment failed (%s)', 'actinia-woocommerce-payment-gateway'), $result['response']['response_status']));
                ...
                if ($this->is_subscription_payment_valid($result['response'], $amount) !== true) {
                    switch ($result['response']['order_status']):
                        case 'approved':
                            ...
                            $order->payment_complete();
                            $order->add_order_note('Actinia subscription payment successful.<br/>actinia ID: ' . ' (' . $result['response']['payment_id'] . ')');

                            return true;
                            break;
                        case 'processing':
                            $order->add_order_note('Transaction ERROR: order in proccesing state<br/>Actinia ID: ' . $result['response']['payment_id']);
                            $order->update_status('failed');

                            return false;
                            break;
                        case 'expired':
                            $order->add_order_note('Transaction ERROR: order expired<br/>Actinia ID: ' . $result['response']['payment_id']);
                            $order->update_status('cancelled');

                            return false;
                            break;
                        case 'declined':
                            $order->add_order_note('Transaction ERROR: order declined<br/>Actinia ID: ' . $result['response']['payment_id']);
                            $order->update_status('cancelled');
    ...
    protected function do_subscription_payment($args)
    {
        ...
        $response = wp_remote_post('https://api.actinia.eu/api/recurring/', $conf);
