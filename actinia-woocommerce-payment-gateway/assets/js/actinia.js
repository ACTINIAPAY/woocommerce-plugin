...
function actinia_submit_order(e) {
    if (jQuery("#payment_method_actinia").is(":checked")) {
        (e || window.event).preventDefault();
        var r = jQuery("form[name=checkout]");
        !r.length && jQuery(".checkout.woocommerce-checkout").length ? r = jQuery(".checkout.woocommerce-checkout") : jQuery("#checkout_actinia_form").find(".error-wrapper").html("Invalid checkout form, please disable on checkout method and try another."), f_clean_error();
        var t = jQuery("#actinia_ccard"), o = jQuery("#actinia_expiry_month"), n = jQuery("#actinia_expiry_year"), a = jQuery("#actinia_cvv2");
        if (console.log(payform.parseCardType(t.val())), console.log(f_valid_credit_card(t.val())), !f_valid_credit_card(t.val()))return actinia_error(t);
        if (!f_valid_month(o.val()))return actinia_error(o);
        if (!f_valid_year(n.val()))return actinia_error(n);
        if (!f_valid_cvv2(a.val()))return actinia_error(a);
        jQuery("#checkout_actinia_form").find(".error-wrapper").hide();
        var c = r.serialize() + "&" + jQuery.param({
                action: "generate_ajax_order_actinia_info",
                nonce_code: actinia_info.nonce
            });
        f_block(jQuery("#checkout_actinia_form")), f_block(jQuery("#place_order")), jQuery.post(actinia_info.url, c, function (e) {
            if ("success" === e.result) {
                jQuery("input[name='token']").length || jQuery("#checkout_actinia_form").append('<input type="hidden" name="token" value=' + e.token + ">");
                var r = {
                    payment_system: "card",
                    token: e.token,
                    card_number: t.val(),
                ...
                $checkout("Api").scope(function () {
                    this.request("api.checkout.form", "request", r).done(function (e) {
                        e.sendResponse(), actinia_post_to_url(e.attr("order").response_url, e.attr("order").order_data, "post")
                    }).fail(function (e) {
                        f_unblock(jQuery("#checkout_actinia_form"));
                        f_unblock(jQuery("#place_order"));
                        var r = e.attr("error").code ? e.attr("error").code : "";
                        jQuery("#checkout_actinia_form").find(".error-wrapper").html(r + ". " + e.attr("error").message).show()
                    })
                })
            } else f_unblock(jQuery("#place_order")), f_unblock(jQuery("#checkout_actinia_form")), jQuery("#checkout_actinia_form").find(".error-wrapper").html(e.messages).show()
        })
    }
}
function f_clean_error() {
    jQuery("#actinia_ccard").removeAttr("style"), jQuery("#actinia_expiry_month").removeAttr("style"), jQuery("#actinia_expiry_year").removeAttr("style"), jQuery("#actinia_cvv2").removeAttr("style")
}
function actinia_error(e) {
...
function actinia_post_to_url(e, r, t) {
    t = t || "post";
...
function actinia_init_card() {
    var e = document.getElementById("actinia_ccard"), r = document.getElementById("actinia_expiry_month"), t = document.getElementById("actinia_expiry_year"), o = document.getElementById("actinia_cvv2");
...
jQuery("#billing_email").length && jQuery("#billing_email").on("input", function () {
    ...
    jQuery("#actinia_ccard").length && actinia_init_card()
});
jQuery(document).ajaxComplete(function () {
    jQuery("#actinia_ccard").keydown(function () {
